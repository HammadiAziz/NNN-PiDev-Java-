    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            for (int i = 0; i < serviceList.size(); i++) {
                ...
                AnchorPane anchorPane = fxmlLoader.load(); // Load the FXML file

                // Get the controller instance from the FXMLLoader
                CardProduct itemController = fxmlLoader.getController();

                // Pass PanierService to the CardProduct constructor
                itemController.setPanierService(panierService);

                // Set data for the CardProduct
                ...
                menu_gridPane.add(anchorPane, column++, row); //(child,column,row)
                //set grid width
