...
public class UserService {
    ...
    public boolean registerUser(User user,   String email,String nom,String password) throws SQLException {
        ...
        try {
            ...
            if (checkResult.next()) {
                ...
                String sql =  "INSERT INTO `user`(`email`, `password`, `first_name`, `last_name`, `phone`, `birthday`, `status`, `roles`, `wallet`) VALUES (?,?,?,?,?,?,?,?,?)";
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    if (resultSet.next()) {
                        ...
                        if (BCrypt.checkpw(password, hashedPassword)) {
                            ...
                            int sessionValue;
                            if (roles.contains("Livreur") || roles.contains("User")) {
    ...
    public boolean editUserInfo(User user) {
        try {
            ...
            String query = "UPDATE user SET first_name = ?, last_name = ?, photo = ? WHERE id = ?";
            try (PreparedStatement preparedStatement = cnx.prepareStatement(query)) {
                preparedStatement.setString(1, user.getNom());
                preparedStatement.setString(2, user.getPrenom());
                preparedStatement.setString(3, user.getPhoto()); // Update the photo field
