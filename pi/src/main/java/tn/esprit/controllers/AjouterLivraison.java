...
public class AjouterLivraison {
    ...
    void AjouterLiv(ActionEvent event) throws SQLException {
        ...
        if (!email.matches("\\b[\\w.%-]+@[\\w.-]+\\.[a-zA-Z]{2,}\\b")) {
            ...
            ls.add(new Livraison(nomText.getText(), prenomText.getText(),  paiementSelectionne,stateText.getText(),adresseText.getText(), emailText.getText(),  Integer.parseInt(phoneText.getText())));
    ...
    void navigatetoAfficherLivAction(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/FXML/User/AfficherLivUser.fxml"));
    ...
    void navigatetoPayment() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/FXML/User/Payment.fxml"));
            nomText.getScene().setRoot(root);
        } catch (IOException e) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setContentText("Sorry");
